    ...
    <div className="bg-[#161618] border border-[#2A2A2C] hover:border-[#D4AF37]/40 transition-all duration-200 rounded-xl p-5 flex gap-4">
      ...
      <div className="flex flex-col items-center justify-start gap-3 min-w-[50px] pt-1">
        <div className="text-center">
          <span className="block text-lg font-bold text-[#D4AF37] font-fira-code">{post.votes_count}</span>
          <span className="text-[10px] text-zinc-500 uppercase tracking-wider font-inter">Votes</span>
        </div>
        <div className="text-center">
          <span className="block text-sm font-semibold text-zinc-300 font-fira-code">{post.comments_count}</span>
          ...
          {post.tags.map((t) => (
            <Link key={t.id} to={`/tags/${t.slug}`}>
              <span 
                ...
                style={{ 
                  backgroundColor: `${t.hex_color || '#D4AF37'}10`, 
                  borderColor: `${t.hex_color || '#D4AF37'}30`,
                  color: t.hex_color || '#D4AF37'
                }}
              >
                #{t.slug}
              </span>
