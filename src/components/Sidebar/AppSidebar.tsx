  ...
  return (
    <>
      ...
      <div className="w-[68px] min-w-[68px] lg:w-[250px] lg:min-w-[250px] h-screen sticky top-0 bg-transparent flex-shrink-0 z-50">
        ...
        <aside
          className={`h-screen bg-[#161618] border-r border-[#2A2A2C] flex flex-col justify-between p-4 font-['Inter'] transition-all duration-300 ease-in-out lg:fixed lg:left-0 lg:top-0 lg:w-[250px] ${
            isExpanded
        ...
        >
          ...
          <div className="flex flex-col gap-6">
            ...
            {/* Toggle Header - Tombol penutup silang/hamburger hanya kerja di mobile, desktop langsung judul */}
            <div
              onClick={() => {
                if (window.innerWidth < 1024) setIsExpanded(!isExpanded);
              }}
              className={`flex items-center h-10 px-1 select-none lg:justify-between ${isExpanded ? "justify-between" : "justify-center"}`}
            >
              {/* Layout Desktop atau Mobile pas lagi kebuka lebar */}
              <div className={`w-full items-center justify-between lg:flex ${isExpanded ? "flex" : "hidden"}`}>
                <div className="text-md font-bold text-[#D4AF37] tracking-tight truncate animate-in fade-in duration-300">
                  <span>Mau Tanya Suhu</span>
                </div>
                {/* Tombol silang penutup (Cuma muncul di HP) */}
                <X className="h-4 w-4 text-gray-400 hover:text-white lg:hidden cursor-pointer" />
              </div>

              {/* Icon Hamburger (Cuma muncul di HP pas sidebar nutup) */}
            ...
            {/* User info - FIX: Nama kiri, Badge USER nembak penuh ke kanan */}
            {user && (
              <div
                className={`flex items-center py-1.5 rounded-lg bg-[#0B0B0C]/40 border border-[#2A2A2C]/40 w-full transition-all duration-300 lg:px-2 lg:gap-3 lg:justify-start ${
              ...
              >
                ...
                >
                  ...
                  {/* Badge Role (Dipaksa mentok kanan pake ml-auto) */}
                  {roleBadge && (
                    <span
                      className={`text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded border ml-auto flex-shrink-0 ${roleBadge.color}`}
                    >
                      {roleBadge.label}
                    </span>
                  )}
